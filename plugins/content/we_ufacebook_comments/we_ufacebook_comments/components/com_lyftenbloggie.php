...
function we_comments_output(&$article,$config,$output){
	if(strpos($article->text,'class="we_fb_comments"')!==false) return false;
	$article->text.=$output;
}
	...
	$selected=false;		
		if (is_array( $display )) {
			foreach ( $display as $key => $value ) {
				if($value=='lyftenbloggie'){ $selected=true; }
			}
		} else {
			if($display=='lyftenbloggie'){ $selected=true; }
		}

#==========================================
	if($selected && $config['get_view']=='entry'){ $flag=true; }
	return $flag;
}
