	...
	protected function getDataQuery($ids = [])
	{
		...
		if ($selects['categories'])
		{
			$query->select([
				...
				$this->config->getTitle('categories_title', 'category_title', 'categories'),
				$this->config->getAlias('categories_alias', 'category_alias', 'categories'),
				$this->config->get('categories_description', 'category_description', 'categories'),
