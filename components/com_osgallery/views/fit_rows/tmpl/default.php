...
$buttons =self::getButtonsArray($db,$galId,$params);
if(count($images)){
    self::addMetaTags();
    ...
    <div class="os-gallery-tabs-main-<?php echo $galId?>">
        ...
        <div class="os-cat-tab-images" >
            ...
            foreach($images as $catId => $catImages){
                ...
                $styleCat = 'style="padding:'.$imageMargin.'px;display:none!important;height: 400px;"';
                ?>
                <!-- Simple category mode-->
                <div class="grid" id="cat-<?php echo $catId?>" data-cat-id="<?php echo $catId?>" <?php echo $styleCat?>>
                   <div class="gutter-sizer"  ></div>
                    ...
                    if($catImages){
                        foreach($catImages as $image){
                            ...
                            <div class="grid-item img-block <?php echo $imageHover ?>-effect" <?php echo $styleImg ?>>
                                ...
                                <a class="os_fancybox-<?php echo $catId?>"
                                    ...
                                    <img src="<?php echo JURI::root()?>images/com_osgallery/gal-<?php echo $image->galId?>/thumbnail_fitrows/<?php echo $image->file_name?>">
    ...
    <script type="text/javascript" charset="utf-8" async defer>
        (function () {
            var osGallery<?php echo $galId?> = function (container, params) {
                ...
                osg.getImgBlockWidth = function ()
                {
                    if(typeof(numColumns) == 'undefined')numColumns = osg.params.numColumns;
                    imgBlockK = 1 / numColumns;
                    var images = jQuerGall(container + " .grid-item");
                    jQuerGall.each(images, function(){
                        var img = jQuerGall(this).find("img");
                        var w_ = jQuerGall(img).prop('naturalWidth');
                        var h_ = jQuerGall(img).prop('naturalHeight');
                        
                        w_ = w_ * imgBlockK;
                        h_ =  h_ * imgBlockK ;

                        img.css('width', w_ + "px");
                        jQuerGall(this).css('width', w_ + "px");
                        img.css('height', h_ + "px");
                        jQuerGall(this).css('height', h_ + "px");
                    ...
                    jQuerGall(".gutter-sizer").css('width', "0%");
                ...
                osg.sortImg = function(catId)
                {
                    if(typeof(numColumns) == 'undefined')numColumns = osg.params.numColumns;
                    var mainBlockW = jQuerGall(container).width();
                    var imgBlockK = 1 / numColumns;
                    var result = [];
                    var images = jQuerGall.makeArray(jQuerGall("#cat-"+ catId +" img"));
                    var min = Math.round(jQuerGall(images[0]).prop('naturalHeight') * imgBlockK);
                    var countImg = Math.round(mainBlockW/min);
                    var counter = images.length;

                    var i = 0;
                    while (i <= counter) {
                        if (countImg == 0){
                          countImg = Math.round(mainBlockW/min);  
                        } 
                        k=Math.round(jQuerGall(images[i]).prop('naturalWidth') * imgBlockK);
                        if (k > min) imgSize = 2;
                        else imgSize = 1;
                        if ((countImg - imgSize) >= 0 && typeof(images[i]) != "undefined" ) {
                            result.push(images[i]);
                            countImg = countImg - imgSize;
                            images.splice(i, 1);
                            counter--;
                            i=0;
                        } else {
                    ...
                    now  = jQuerGall("#cat-"+ catId+" img");

                    for (var i = 0; i < now.length; i++) {
                        for (var j = 0; j < result.length; j++) {
                            if(now[i].src == result[j].src) {
                                result[j] = jQuerGall(now[i]).closest(".grid-item").get(0);
                            }
                        };
                    };

                    jQuerGall("#cat-"+catId).append(result);
                }

                osg.isotope = function(curCatId)
                {
                    jQuerGall(container + ' .grid').imagesLoaded(function(){
                        osg.sortImg(curCatId);
                        osg.getImgBlockWidth();
                        ...
                        if(catId){
                            jQuerGall(this).find(".os_fancybox-"+catId).os_fancybox({
                                ...
                                afterShow: function() {
                                    ...
                                    var id = this.element.attr('id');
                                    var naturalWidth = jQuerGall("#img-" + id.split('-')[1] + " img").prop('naturalWidth') ;
                                    var naturalHeight = jQuerGall("#img-" + id.split('-')[1] + " img").prop('naturalHeight') ;
                                    ...
                                    jQuerGall("#img-" + id.split('-')[1] + " img").css({'width': naturalWidth, 'max-width': '100%'} );
                                    jQuerGall("#img-" + id.split('-')[1] + " img").css({'height': naturalHeight, 'max-height': '100%'} );
